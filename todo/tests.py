...
class TaskModelTestCase(TestCase):
    ...
    def test_is_overdue_past(self):
        """【追加ケース 1】締切を過ぎている場合のテスト"""
        # "2024/6/30 23:59:59" 締切のタスク
        due = timezone.make_aware(datetime(2024, 6, 30, 23, 59, 59))
        # 現在時刻: "2024/7/1 0時" (締切を過ぎている)
        current = timezone.make_aware(datetime(2024, 7, 1, 0, 0, 0))
        task = Task(title='task2', due_at=due)
        task.save()

        # 締切を過ぎているので True を返すことを検証
        self.assertTrue(task.is_overdue(current))

    def test_is_overdue_none(self):
        """【追加ケース 2】締め切りのないタスクのテスト"""
        # 締め切りのないタスク（due_at を None に設定、あるいは指定しない）
        current = timezone.make_aware(datetime(2024, 7, 1, 0, 0, 0))
        task = Task(title='task3', due_at=None)
        task.save()

        # 締め切りがない場合は False を返すことを検証
        self.assertFalse(task.is_overdue(current))
